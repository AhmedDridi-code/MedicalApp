...
public class RendezVousActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_rendez_vous);
            ...
            recyclerView = findViewById(R.id.appoitments_recycler);
                    ...
                    try{
                        if(response.length()>0){
                            for (int i = 0; i < response.length(); i++) {
                                ...
                                appointment.setDate(json.getString("date"));
                                appointment.setTitle(json.getString("titre"));
                                appointment.setTime(json.getString("heure"));
                                appointments.add(appointment);
                            }
                            Log.i("debug", "in if statement");
                            recyclerView.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
                            adapter = new AppointmentsAdapter(getApplicationContext(),appointments);
